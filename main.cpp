...
int main(int argc, char const *argv[])
{
    ...
    detector->init(xml_path,0.19,0.5);
    /*
    ...
    */  
    ...
    for(int i=0;i<detected_objects.size();++i){
        ...
        int height = detected_objects[i].rect.height;
        Rect rect(xmin, ymin, width, height);//左上坐标（x,y）和矩形的长(x)宽(y)
        cv::rectangle(osrc, rect, Scalar(200, 0, 255),2, LINE_AA,0);
        cv::putText(osrc,detected_objects[i].name,cv::Point(rect.x,rect.y - 5),2,1,Scalar(200, 0, 255),1,LINE_AA);
    }   
    imshow("result",osrc);
    waitKey(0);
}
